...
def main():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            page.wait_for_load_state("networkidle")  # 검색 후 네트워크 유휴 상태 대기
            ...
            print(f"[INFO] '{search_query}' 링크를 클릭하여 페이지를 이동합니다. (동일 탭)")
            link_href = result_locator.first.get_attribute("href")
            if link_href:
                page.goto(link_href, wait_until="networkidle")  # 동일 탭에서 페이지 이동
            else:
            ...
            for i, marker_span in enumerate(marker_spans):
                print(f"[INFO] {i+1}/{len(marker_spans)}번째 매물 마커 클릭 시도...")
                try:
                    marker_span.click()
                    # 클릭 후 페이지 내용이 업데이트될 때까지 대기 (필요시 추가적인 대기 로직)
                    page.wait_for_load_state("networkidle") # 페이지 내용이 로드될 때까지 대기

                    # HTML에서 item--child 클래스 찾기 (각 매물 항목)
                    item_child_elements = page.locator(".item--child").all()
                    if item_child_elements:
                        print(f"[INFO] 페이지 내에서 {len(item_child_elements)}개의 매물 항목을 찾았습니다.")
                        for item_idx, item_element in enumerate(item_child_elements):
                            # 각 항목에서 필요한 정보 추출
                            owner_place_text = item_element.locator("em.title_place").inner_text() if item_element.locator("em.title_place").count() > 0 else ""
                            transaction_type_text = item_element.locator(".price_area .type").inner_text() if item_element.locator(".price_area .type").count() > 0 else ""
                            price_text = item_element.locator("strong.price").inner_text() if item_element.locator("strong.price").count() > 0 else ""
                            
                            info_elements = item_element.locator(".information_area p.info").all()
                            info1_text = info_elements[0].inner_text() if len(info_elements) > 0 else ""
                            info2_text = info_elements[1].inner_text() if len(info_elements) > 1 else ""
                            
                            tag_area_tags = ", ".join(item_element.locator(".tag_area .tag").all_inner_texts())
                            
                            confirmation_date_text = item_element.locator("span.icon-badge.type-confirmed").inner_text() if item_element.locator("span.icon-badge.type-confirmed").count() > 0 else ""
                            link_href = item_element.locator("a.item_link").get_attribute("href") if item_element.locator("a.item_link").count() > 0 else ""
                            ...
                            print(f"[SUCCESS] 매물 항목 추출 완료 ({item_idx+1}) - 가격: {price_text}, 유형: {transaction_type_text}")
                    else:
                        print("[WARNING] 페이지 내에서 매물 항목 (item--child)을 찾을 수 없습니다.")
                    print("[SUCCESS] 내용 추출 완료.")

                except Error as e:
                    print(f"[ERROR] 매물 마커 클릭 또는 내용 추출 중 오류 발생: {e}")
            ...
            if extracted_data:
                with open(csv_file_path, "w", newline="", encoding="utf-8") as csvfile:
                    fieldnames = ["marker_index", "집주인", "타입", "가격", "정보1", "정보2", "tag_area", "확인매물 날짜", "link_href"]
                    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
                    writer.writeheader()
                    writer.writerows(extracted_data)
                print(f"[SUCCESS] 추출된 데이터가 '{csv_file_path}'에 저장되었습니다.")
            else:
