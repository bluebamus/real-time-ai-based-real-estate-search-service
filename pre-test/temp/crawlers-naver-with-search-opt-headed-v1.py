...
class NaverRealEstateScraper:
    ...
    def set_search_options(
        ...
        sale_price: list[int] | None,  # Added sale_price
        ...
        if transaction_type:
            ...
            for tt in to_deselect:
                try:
                    ...
                    label_locator = self.page.locator(f"div.article_box--option:not(._complexFilterBox) div._multiFilter[filtername='tradTpCd'] input[headernm='{tt}'] + label")
                    if label_locator.count() > 0:
                        label_locator.first.click()
                        print(f"[SUCCESS] 거래유형 '{tt}' 선택 해제.", flush=True)
                        time.sleep(0.5)
                    else:
                        print(f"[DEBUG] 거래유형 '{tt}' 요소를 찾을 수 없음", flush=True)
                except Error as e:
        ...
        if building_type:
            ...
            for bt in to_deselect:
                if bt not in processed_building_types:
                    try:
                        # 메인 필터에서만 클릭 (미니 필터 제외하고 표시되는 필터만)
                        label_locator = self.page.locator(f"div.article_box--option:not(._complexFilterBox) div._multiFilter[filtername='rletTpCd'] input[headernm='{bt}'] + label")
                        if label_locator.count() > 0:
                            label_locator.first.click()
                            ...
                            print(f"[DEBUG] 매물유형 '{bt}' 요소를 찾을 수 없음", flush=True)
                    except Error as e:
                        print(f"[WARNING] 매물유형 '{bt}' 선택 해제 실패: {e}", flush=True)
        ...
        if sale_price:
            ...
            try:
                if len(sale_price) == 1:
                    ...
                    print(f"[SUCCESS] 매매가 최대값 '{sale_price[0]}' 직접 입력 성공.", flush=True)
                elif len(sale_price) == 2:
                    self.page.locator("#dprcMin").fill(str(sale_price[0]))
                    self.page.locator("#dprcMax").fill(str(sale_price[1]))
                    print(f"[SUCCESS] 매매가 범위 '{sale_price[0]} ~ {sale_price[1]}' 직접 입력 성공.", flush=True)
                time.sleep(0.5)
            except Error as e:
        ...
        if deposit:
            ...
            try:
                if len(deposit) == 1:
                    ...
                    print(f"[SUCCESS] 보증금 최대값 '{deposit[0]}' 직접 입력 성공.", flush=True)
                elif len(deposit) == 2:
                    self.page.locator("#wprcMin").fill(str(deposit[0]))
                    self.page.locator("#wprcMax").fill(str(deposit[1]))
                    print(f"[SUCCESS] 보증금 범위 '{deposit[0]} ~ {deposit[1]}' 직접 입력 성공.", flush=True)
                time.sleep(0.5)
            except Error as e:
        ...
        if monthly_rent:
            ...
            try:
                if len(monthly_rent) == 1:
                    ...
                    print(f"[SUCCESS] 월세 최대값 '{monthly_rent[0]}' 직접 입력 성공.", flush=True)
                    ...
                    print(f"[SUCCESS] 월세 범위 '{monthly_rent[0]} ~ {monthly_rent[1]}' 직접 입력 성공.", flush=True)
                time.sleep(0.5)
            except Error as e:
        ...
        if area_range:
            ...
            if area_option:
                print(f"[INFO] 면적대: '{area_range}' -> '{area_option}' 클릭", flush=True)
                try:
                    self.page.locator("#filterLayer #ct").get_by_role("listitem").filter(has_text=area_option).locator("label").click()
                    print(f"[SUCCESS] 면적대 '{area_option}' 클릭 성공.", flush=True)
                    time.sleep(0.5)
                except Error as e:
                    print(f"[WARNING] 면적대 '{area_option}' 클릭 실패: {e}", flush=True)

        # 7. 매물검색 버튼 클릭
        print("[INFO] 매물검색 버튼을 클릭합니다.", flush=True)
        try:
            # dump.html에서 확인한 정확한 선택자 사용
            search_button = self.page.locator("a.btn_option.btn_option--search._filterSaveBtn")
            ...
            # 대안 선택자 시도
            try:
                print("[INFO] 대안 선택자로 매물검색 버튼 클릭을 재시도합니다.", flush=True)
    ...
    def _convert_price_to_text(self, price: int) -> str:
        """
        숫자 가격을 '1천', '1억'과 같은 텍스트 형식으로 변환합니다.
        """
        if price >= 100000000:
            return f"{price // 100000000}억"
        elif price >= 10000000:
            return f"{price // 10000000}천"
        elif price >= 10000:
        ...
        if len(parts) > 0:
            ...
            if "/" in area_part and "㎡" in area_part:
                try:
                    ...
                    print(
                        f"[DEBUG]  - 사양 (평수 변환): '{sq_meter_str}㎡' -> '{pyeong}'평"
                    )
                except (ValueError, IndexError) as e:
    ...
    def _extract_data_from_item(self, item_element) -> dict:
        ...
        if not data["집주인"]:
            print(
                "[DEBUG] '집주인' 필드가 비어 있어 이 아이템을 건너뜁니다 (광고 또는 비정상 아이템)."
            )
            return {}
            ...
            try:
                ...
                for item_element in item_elements:
                    try:
                        ...
                        print(
                            "[DEBUG] article_no를 가져올 수 없는 아이템을 건너뜁니다 (광고 가능성)."
                        )
                        continue
...
if __name__ == "__main__":
    ...
    try:
        ...
        scraper.set_search_options(
            ...
            sale_price=keywords["sale_price"],  # Added sale_price
