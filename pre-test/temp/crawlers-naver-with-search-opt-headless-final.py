...
class NaverRealEstateScraper:
    """
    네이버 부동산 웹사이트에서 부동산 매물 정보를 스크래핑하는 클래스 (Headless 버전).
    """
    ...
    def __init__(self, headless: bool = True):
        ...
        :param headless: 브라우저를 headless 모드로 실행할지 여부. (기본값: True)
        """
    ...
    def initialize_browser_and_page(self):
        ...
        쿠키 및 브라우저 설정을 포함합니다. (탐지 방지 설정 강화)
        """
        print("[INFO] Playwright 및 브라우저 초기화를 시작합니다. (Headless 모드)", flush=True)
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                self.context = self.browser.new_context(
                    ...
                    # 이미지 로딩 비활성화로 속도 향상
                    # extra_http_headers에서 이미지 차단은 제거 (일부 사이트에서 문제 발생 가능)
                )
...
if __name__ == "__main__":
    print("==================================================", flush=True)
    print(
        "[INFO] 네이버 부동산 자동 검색 프로그램을 시작합니다. (Headless 버전)",
        flush=True,
    )
    print("==================================================", flush=True)

    # Headless 모드로 설정
    scraper = NaverRealEstateScraper(headless=True)
    ...
    try:
        ...
        scraper.scrape_all_markers_and_save_csv("popup_data_headless.csv")
        time.sleep(2)
