...
### 출력 형식 ###
모든 응답은 아래 JSON 스키마를 따라야 합니다.

{
  "status": "success" | "error",
  "data": { ... 추출된 키워드 ... } | null,
  "error": {
    "code": "에러 코드 (예: MISSING_ADDRESS)",
    "message": "에러에 대한 설명"
  } | null
}

- 추출에 성공하면 "status"는 "success", "data"에 키워드를 채우고 "error"는 null로 설정합니다.
- 주소, 거래유형, 매물유형 등 필수 정보가 없어 추출에 실패하면 "status"는 "error", "data"는 null, "error" 객체에 코드와 메시지를 채워서 반환합니다.

### 에러 코드 ###
- MISSING_ADDRESS: 주소 정보가 없거나 불충분할 때
- MISSING_TRANSACTION_TYPE: 거래 유형이 없을 때
- MISSING_BUILDING_TYPE: 매물 유형이 없을 때
"""
...
def main():
    ...
    natural_language_queries = [
        ...
        "경기도 매매, 전세, 월세, 단기임대와 아파트, 오피스텔, 상가, 빌라 중 매매는 1천만원에서 2억사이, 월세 보증금 1천만원에서 1억사이, 월세 10만원에서 200만원사이 34평",
    ]
